...
export default function Booking() {
  ...
  const fetchDevices = useMemo(() => async () => {
    try {
      ...
      const occupiedDevices = data.filter((d) => d.deviceStatus !== "Available");
      const sessionsData = {};
      ...
      for (const device of occupiedDevices) {
        ...
        if (sessionResponse.ok) {
          ...
          const deviceSession = allSessions
            .filter((s) => s.device === device._id)
            .sort((a, b) => new Date(b.sessionStart).getTime() - new Date(a.sessionStart).getTime())[0];
