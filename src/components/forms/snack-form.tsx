...
import type { Snack } from "@/components/snacks/snack-list";

interface SnackFormProps {
  snack?: Snack | null;
  onSuccess: () => void;
}
type SnackFormData = Omit<Snack, '_id'>;

export function SnackForm({ snack, onSuccess }: SnackFormProps) {
  ...
  const onSubmit = async (data: SnackFormData) => {
    try {
      setLoading(true);
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: error.message || "Error creating snack"
      });
